  ...
  <form action="{{ route('eventos.update', $evento) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
    ...
    <div class="grid sm:grid-cols-2 gap-4">
      <div>
        ...
        <input type="date" name="fecha_inicio" class="w-full border rounded-md p-2"
          value="{{ $evento->fecha_inicio->format('Y-m-d') }}" required>
      </div>
      <div>
        <label class="block font-medium">Fecha fin</label>
        <input type="date" name="fecha_fin" class="w-full border rounded-md p-2"
          value="{{ optional($evento->fecha_fin)->format('Y-m-d') }}">
      </div>
    </div>
